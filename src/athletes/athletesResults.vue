    ...
    <b-card v-if="resultsData" title="Results" tag="article" class="mb-2">
      ...
      >
        <template slot="event" slot-scope="props">
          <b-button size="sm" variant="light" @click="eventClick(props.rowData.event.id)">
...
<script>
import FieldsDefsAthleteResults from './FieldsDefsAthleteResults.js'
...
export default {
  data () {
    return {
      fieldsAthleteResults: FieldsDefsAthleteResults,
      css: CssConfig,
      resultsData: [],
